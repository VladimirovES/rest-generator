...
class CustomModelGenerator:
    ...
    def generate_models_for_endpoint(
        self, endpoint_path: str, method: str, endpoint_dir: str
    ) -> List[str]:
...
'''
    ...
    def _generate_models_init(
        self, models_dir: str, model_names: Iterable[str]
    ) -> None:
        ...
        def _scan_text(value: str) -> None:
            for candidate in re.findall(r"\b[A-Z][a-zA-Z0-9_]*\b", value):
                ...
                if candidate == model_def.name:
                    continue
                if candidate in self.all_models and not self._should_skip_model(
                    candidate
                ):
    ...
    def _generate_type_alias(self, model_def: ModelDefinition) -> List[str]:
        ...
        lines.append(
            f'{model_def.name} = {model_def.base_type.replace("TypeAlias = ", "")}'
        )
    ...
    def _generate_field_definition(
        self, field, current_class_name: str = ""
    ) -> List[str]:
