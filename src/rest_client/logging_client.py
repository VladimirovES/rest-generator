...
class Logging:
    ...
    def log_request(self, method: str, url: str, **kwargs: Any) -> None:
        json_data = self._extract_json_data(
            kwargs.get("json"), kwargs.get("content"), kwargs.get("data")
        )

        request_info = {"event": "request", "method": method, "url": url}
    ...
    def log_curl(self, response) -> None:
        ...
        allure.attach(
            curl, name="cURL Command", attachment_type=allure.attachment_type.TEXT
        )
