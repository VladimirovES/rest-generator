...
class EnhancedClientGenerator(ClientGenerator):
    ...
    def generate_clients_with_models(self, output_dir: str) -> Dict[str, str]:
        ...
        for file_name, endpoints in file_to_endpoints.items():
            ...
            self._generate_module_client_file(
                module_dir, endpoints, class_name, all_model_names
            )
    ...
    def _generate_module_client_file(
        self,
        module_dir: str,
        endpoints: List[Endpoint],
        class_name: str,
...
'''
    ...
    def _generate_service_init(
        self, service_dir: str, file_to_class: Dict[str, str]
    ) -> None:
    ...
    def _render_template(self, template_data: Dict[str, Any]) -> str:
        ...
        if (
            "methods" in template_data
            and template_data["methods"]
            and isinstance(template_data["methods"][0], Endpoint)
        ):
            template_data["methods"] = [
                MethodContext.from_endpoint(ep) for ep in template_data["methods"]
            ]
